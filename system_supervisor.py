...
#This file contains the implementation of the system supervisor
...
#When an instance starts, it sends a poll message to the system supervisor. A poll message is an object created from the following class.
class poll_message:
	...
	while(1):
		...
		if(command[0]=="exit"):
			...
			for i in range(0,len(cluster_nodes)):
				...
				sndmsg_initial=agent_message_class(LEADER_DELETE_CONTAINER_TYPE,components)
				sndmsg_initial=pickle.dumps(sndmsg_initial)
				
				#Send a message with the following structure: HEADER(20 Bytes) + pickled object
				delete_message=bytes(f'{len(sndmsg_initial):<{HEADER_LENGTH}}',"utf-8") + sndmsg_initial
				sock.sendall(delete_message)
			...
			migrate(command[1],command[3],command[5])
		else:
			print("Invalid command")
			
			
def migrate(component_to_move,from_device,to_device):
	...
	for i in range(0,len(cluster_nodes)):
		if(cluster_nodes[i][0]==to_device):
			...
			sndmsg_initial=agent_message_class(LEADER_CREATE_SERVICE_TYPE,bashCmd)
			sndmsg_initial=pickle.dumps(sndmsg_initial)
			
			#Send a message with the following structure: HEADER(20 Bytes) + pickled object
			command_message=bytes(f'{len(sndmsg_initial):<{HEADER_LENGTH}}',"utf-8") + sndmsg_initial
	...
	if(left_reliability=="reliable"):
		...
		inform_instances(1)
		print("Reliable ingress link, send a START_MOVE_BUFFER_TYPE message")
	...
	if(right_reliability=="reliable"):
		print("Reliable egress link, send a CLOSE_DESTINATION_CONNECTIONS_TYPE message")
	...
	for i in range(0,len(cluster_nodes)):
		if(cluster_nodes[i][0]==from_device):
			...
			sndmsg_initial=agent_message_class(LEADER_DELETE_CONTAINER_TYPE,[component_to_move])
			sndmsg_initial=pickle.dumps(sndmsg_initial)
	...
	for i in range(0,len(instances_table)):
		...
		for j in range(0,len(instances_table[i][4])):
			...
			if(instances_table[i][4][j].get_changed_state()==1):
				send_parser_message(instances_table[i][4][j].get_instance_name(),instances_table[i][0],LEADER_MESSAGE_TYPE_MOVE_INITALIZATION)
...
def main(config_name):
	...
	while(1):
		...
		sndmsg_initial=cluster_message_class(INFORMATION_TYPE)
		sndmsg_initial=pickle.dumps(sndmsg_initial)
		...
		#Set the current service's running_instances_table
		for i in range(0,len(cluster_nodes)):
			if(cluster_nodes[i][2]==0):
				missing_devices.append(cluster_nodes[i][0])
				print("One or more devices are missing")
		...
		for i in range(0,len(cluster_nodes)):
			...
			sndmsg_initial=agent_message_class(LEADER_PING_TYPE,"")
			sndmsg_initial=pickle.dumps(sndmsg_initial)
			
			#Send a message with the following structure: HEADER(20 Bytes) + pickled object
			ping_message=bytes(f'{len(sndmsg_initial):<{HEADER_LENGTH}}',"utf-8") + sndmsg_initial
		...
		if(len(current_service.get_component_replicas_table())==0):
			...
			for j in range(0,len(components)):
				...
				if(len(missing_devices)>0):
					print("REMOVE MISSING DEVICES FROM COMPONENT REPLICAS TABLE")
					current_service.remove_missing_devices_from_component_replicas_table(components[j],missing_devices)
			...
			if(len(missing_devices)>0):
				...
				for k in range(0,len(missing_devices)):
					
					migrated_instances=0
					instances_to_migrate=0
					can_not_migrate=0
					for i in range(0,len(running_instances_table)):
						if(missing_devices[k]==running_instances_table[i][0]):
							
							for j in range(0,len(running_instances_table[i][4])):
								
								print("SHOULD MIGRATE THIS RUNNING INSTANCE",running_instances_table[i][4][j].get_instance_name())
								instances_to_migrate+=1
								free_compatible_devices=current_service.get_compatible_devices_without_running_instances(running_instances_table[i][4][j].get_instance_name())
								
								print("free_compatible_devices",free_compatible_devices)
									
								if(len(free_compatible_devices)==0):
									print("There are not any available devices for this running instance to migrate")
									can_not_migrate=1
									break
								
							if(can_not_migrate==1):
								break

						
						
					if(can_not_migrate==0):
						
						
						for i in range(0,len(running_instances_table)):
							if(missing_devices[k]==running_instances_table[i][0]):
								
								for r in range(0,len(running_instances_table[i][4])):
									for j in range(0,len(running_instances_table[i][4])):
										
										print("MIGRATE THIS RUNNING INSTANCE",running_instances_table[i][4][j].get_instance_name())
										free_compatible_devices=current_service.get_compatible_devices_without_running_instances(running_instances_table[i][4][j].get_instance_name())
										print("free_compatible_devices",free_compatible_devices)
										
										ret=migrate(running_instances_table[i][4][j].get_instance_name(),missing_devices[k],free_compatible_devices[0])
					...
					if(can_not_migrate==1 or migrated_instances!=instances_to_migrate):
						
						print("Give the device wants to exit the cluster a nack")
						sock=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
						sock.connect(cluster_leader_address)
						
						print(missing_devices[k])
						
						sndmsg_initial=cluster_message_class(CANCEL_EXIT_TYPE,missing_devices[k])
						sndmsg_initial=pickle.dumps(sndmsg_initial)
						...
						device_address = (current_service.get_ip(missing_devices[k]),AGENT_PORT)
						sock=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
						sock.connect(device_address)
						
						
						sndmsg_initial=agent_message_class(EXIT_REPLY_NACK_TYPE,missing_devices[k])
						sndmsg_initial=pickle.dumps(sndmsg_initial)
						
						#Send a message with the following structure: HEADER(20 Bytes) + pickled object
						agent_message=bytes(f'{len(sndmsg_initial):<{HEADER_LENGTH}}',"utf-8") + sndmsg_initial
						sock.sendall(agent_message)
					...
					elif(can_not_migrate==0 and migrated_instances==instances_to_migrate):
						
						
						print("Give the device wants to exit the cluster an ack")
						sock=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
						sock.connect(cluster_leader_address)
						
						print(missing_devices[k])
						
						sndmsg_initial=cluster_message_class(APPROVED_EXIT_TYPE,missing_devices[k])
						sndmsg_initial=pickle.dumps(sndmsg_initial)
						...
						sndmsg_initial=agent_message_class(EXIT_REPLY_ACK_TYPE,missing_devices[k])
						sndmsg_initial=pickle.dumps(sndmsg_initial)
						...
						current_service.delete_device(missing_devices[k])
						devices.remove(missing_devices[k])
